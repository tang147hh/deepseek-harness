...
/**
 * Native desktop actions stay loopback-only even when a platform gateway has
 * already authenticated the browser. `pickDirectory` / `openPath` drive the
 * host OS; they must not become reachable just because Host is a trusted
 * authority.
 */
...
/**
 * Extra Host authorities accepted for configuration-plane methods.
 *
 * Default is `[]` (loopback pin). Platform plate F sets `DSH_TRUST_GATEWAY=1`
 * in the per-user container: the app gateway already required a session, and
 * Host must still match `trustedHosts` (`--trusted-host $APP_HOST`). Native
 * pickDirectory / openPath stay loopback-only.
 */
function privilegedTrustHosts(method: string, trustedHosts: readonly string[]): readonly string[] {
  if (NATIVE_DESKTOP_METHODS.has(method)) return []
  if (process.env.DSH_TRUST_GATEWAY === '1') return trustedHosts
  return []
}
  ...
  const fetchHandler = connection.createSharedFetchHandler(API_PATH, {
    async fetch(request) {
      ...
      if (method !== undefined
        ...
        && !isTrustedApiRequest(request, privilegedTrustHosts(method, trustedHosts))) {
