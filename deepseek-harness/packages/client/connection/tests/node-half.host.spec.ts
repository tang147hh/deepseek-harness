...
describe('connection node half', () => {
  ...
  it('lets a gateway-trusted Host reach settings/credentials but not native desktop methods', async () => {
    // Plate F: DSH_TRUST_GATEWAY=1 + Host in trustedHosts unlocks the
    // configuration plane so Models can write a key through the app gateway.
    // host.pickDirectory / host.openPath stay loopback-only.
    vi.stubEnv('DSH_TRUST_GATEWAY', '1')
    const { routes, dispose } = await mounted({ trustedHosts: ['app.localhost'] })
    try {
      for (const method of [
        'settings.describe', 'settings.openDocument', 'settings.update', 'settings.replace', 'settings.mutate',
        'credentials.describe', 'credentials.set', 'credentials.unset',
        ...
        await routes[0]!.handler(
          fakeRequest({ host: 'app.localhost' }, `${API_PATH}/${method}`),
          allowed.response,
        )
        expect(allowed.state.status).not.toBe(403)
      }
      for (const method of ['host.pickDirectory', 'host.openPath']) {
        const denied = fakeResponse()
        await routes[0]!.handler(
          fakeRequest({ host: 'app.localhost' }, `${API_PATH}/${method}`),
          denied.response,
      ...
      await routes[0]!.handler(
        fakeRequest({ host: 'evil.example' }, `${API_PATH}/credentials.set`),
        untrusted.response,
      )
